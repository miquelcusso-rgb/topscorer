...
async function main() {
  ...
  // ── Anti-wipe guard ──────────────────────────────────────────────────────
  // The API free tier (100 req/day) can rate-limit a whole run; apiFetch warns
  // per-league and continues, so a failed run could collapse to 0 rows and
  // OVERWRITE the dataset with an empty array (this happened on a weekly cron).
  // Refuse to write when the result is implausibly small or a big regression
  // vs the existing file — keep the previous data and fail loudly instead.
  const FLOOR = 300
  let prevCount = 0
  try {
